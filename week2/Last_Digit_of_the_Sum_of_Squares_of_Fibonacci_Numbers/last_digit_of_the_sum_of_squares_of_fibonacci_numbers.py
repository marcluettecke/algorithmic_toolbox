...
def last_digit_sequence_sum_squared():
    """
    ...
    running_sums_squared = [sum(initial_values[:i]) ** 2 for i in range(1, len(initial_values) + 1)]

    # print(list(map(modulo10, running_sums_squared)))
    return list(map(modulo10, running_sums_squared))
...
def last_digit_of_the_sum_of_squares_of_fibonacci_numbers(n):
    ...
    sequence_of_digits = last_digit_sequence_sum_squared()
    return sequence_of_digits[n % 60]


if __name__ == '__main__':
    # last_digit_sequence_sum_squared()
    input_n = int(input())
